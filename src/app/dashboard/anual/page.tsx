  ...
  return (
    <span className={cls} aria-hidden="true">
      <svg viewBox="0 0 24 24">
        {direction === "up" ? (
          <path d="m12 4 6.5 7h-4v9h-5V11h-4z" fill="currentColor" />
        ) : (
          <path d="M9.5 4h5v9h4L12 20l-6.5-7h4z" fill="currentColor" />
        )}
      </svg>
    </span>
  );
    ...
    <div className="casha-wrap">
      <div className="casha-shell">
        ...
        <div className="workspace-shell">
          <DashboardSidebar current="annual" selectedMonth={selectedMonth} lang={lang} currency={currency} showBottomControls userDisplayName={name} logoutLabel={lang === "pt-PT" ? "Terminar sessão" : "Logout"} />
                ...
                {categoryRows.length ? (
                  <ul className="breakdown-list">
                    {categoryRows.map((row, index) => {
                      ...
                      const barColor = annualBreakColors[index % annualBreakColors.length];
                      return (
                        <li key={row.category}>
                          ...
                          <div className="break-track"><div style={{ width: `${pctValue}%`, background: barColor }} /></div>
                          <small>{Math.round(pctValue)}%</small>
                        </li>
                      );
                    })}
                  </ul>
